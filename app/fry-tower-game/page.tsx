...
export default function FryTowerGame() {
  ...
  const iframeSrc = mode === 'multi' && GAME_SERVER_URL
    ? `/fry-tower-game/index.html?mode=multi&server=${encodeURIComponent(GAME_SERVER_URL)}`
    : '/fry-tower-game/index.html';

  if (mode === 'select') {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-yellow-500 via-amber-600 via-orange-700 to-red-800">
        <Link
          href="/"
        ...
        <div className="flex flex-col items-center gap-8">
          <h1 className="text-5xl font-bold text-white">🍟 Fryffel Tower</h1>
          <p className="text-xl text-amber-200">플레이 모드를 선택하세요</p>
          <div className="flex gap-6">
            <button
              onClick={() => setMode('single')}
              className="px-8 py-4 bg-gradient-to-br from-yellow-500 to-orange-600 text-white text-xl font-bold rounded-2xl shadow-lg hover:scale-105 transition-transform"
            >
              싱글플레이어
            </button>
            <button
          ...
          {!GAME_SERVER_URL && (
            <p className="text-sm text-red-300">서버가 설정되지 않았습니다</p>
          )}
        </div>
      </div>
    );
