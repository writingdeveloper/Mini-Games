...
export default function FlightGame() {
  ...
  const iframeSrc = mode === 'multi' && GAME_SERVER_URL
    ? `/flight-game/index.html?mode=multi&server=${encodeURIComponent(GAME_SERVER_URL)}`
    : '/flight-game/index.html';

  if (mode === 'select') {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-cyan-900 via-blue-900 to-indigo-900">
        <Link
          href="/"
        ...
        <div className="flex flex-col items-center gap-8">
          <h1 className="text-5xl font-bold text-white">Sky Explorer</h1>
          <p className="text-xl text-cyan-200">플레이 모드를 선택하세요</p>
          <div className="flex gap-6">
            <button
              onClick={() => setMode('single')}
              className="px-8 py-4 bg-gradient-to-br from-cyan-500 to-blue-600 text-white text-xl font-bold rounded-2xl shadow-lg hover:scale-105 transition-transform"
            >
              싱글플레이어
            </button>
            <button
