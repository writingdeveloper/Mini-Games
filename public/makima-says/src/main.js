...
function nextRound() {
  ...
  // floor 0.8s so the (now-shortened ~0.7-0.77s) 레제 voice always finishes inside the window — synced, not cut off
  const win = (command.speaker === "reze" ? Math.max(0.8, 0.95 - round * 0.02) : reactionWindow(round)) * 1000;
