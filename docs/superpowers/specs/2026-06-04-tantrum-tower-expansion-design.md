...
### 5.3 QA 패스 (게이머 페르소나 + 전문가 5인 검토 → 수정 A–E, ✅ 완료)
S6 후 **상세 플레이 QA**(직접 라이브 플레이 + 게임디자인·UX·코드정확성·테스트·퍼포먼스 5개 렌즈 병렬 검토)를 수행. 핵심 발견과 수정(브랜치 `feat/tantrum-tower-qa-fixes`, plan `docs/.../plans/2026-06-04-tantrum-tower-qa-fixes.md`):
- **A 이벤트 공정성:** 고정 시드(`seed+777` 상수) → **매판 동일 + 악재(고장×2) 도입부**가 최대 문제였음. → 세션별 재시드(`+ _session*2654435761`, 여전히 결정적·e2e 무위험) + **첫 이벤트 항상 호재/중립**(그레이스) + **악재 2연속 금지** 가드(`pickEventGuarded`).
- **B 피드백:** 단일 토스트 1칸이 이벤트/보상 메시지를 서로 덮어씀 → **이벤트/보상 2채널 분리 + 채널별 큐**. 진행 중 효과 비표시 → **상태 칩**(`🔧 −50% 8s`/`⚡ +36% 4s`) + **진행바 색 틴트**(빨강/초록).
- **C 테스트성:** 와이어링이 무테스트였음 → `applyEventEffects`/`tickEventMultipliers`/`initEventState`를 **순수 함수로 추출**(THREE-free 단위 테스트) + `?eventDelay=` **e2e 훅**으로 런타임 경로 스모크.
- **D 밸런스:** **drill(박 군기) 함정 카드 제거**(고용비 2000→1400·월급 12→8); **난이도별 이벤트 배수**(easy 악재 0.6·호재 1.2 / hard 1.3·0.9); `breakdownSec` 14→9; **accident가 도주 임계 이상 일꾼 제외**(무경고 폭동 방지).
- **E 접근성:** **시작화면 이벤트 범례** + **색맹 안전 중복코딩**(▲/▼/◆ + 색); **관리자 로스터 HUD**(디오라마 거리 가독성) + **고용 모달**(2×2, 플롯 겹침 해소); **reduced-motion**(정점 지터·폭동 바운스·전투 푸시인 + CSS) + **오디오 음소거/볼륨**(localStorage 영속).
- **검증:** 단위 79/79·e2e 4/4(신규 이벤트 스모크 포함)·lint·type-check 그린; 라이브 플레이로 전 항목 시각 확인(첫 이벤트가 호재로 바뀜·칩/틴트·범례·로스터·모달·토글), 콘솔 에러 0. 11커밋, 각 태스크 2단계 리뷰(스펙→코드품질). 페이롤 "이상"은 **버그 아님**으로 확정(HUD 텍스트 freeze 아티팩트).
