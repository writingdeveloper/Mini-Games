...
| ID | 스토리 | 상태 | 우선 | 의존 |
|---|---|---|---|---|
| **S1** | 경제 & 인건비 (자금 획득·관리자 월급 지출) | 📋 설계됨(이 문서 §2.1) | ★ 1 | v1 |
| **S2** | AI 관리자 4종 고용 (개성·자동 관리·외형 차별화) | 📋 설계됨(§2.2) | ★ 1 | S1 |
| **S3** | 난이도 모드(Easy/Normal/Hard) + 관리자 레버 | 📋 설계됨(§2.3) | ★ 1 | S1·S2 |
| **S4** | 다중 건물 (단일 현장 → 여러 건물 순차/동시) | ✏️ 스케치(§5.1) | 2 | S1·S3 |
| **S5** | 에셋 패스 (Quaternius 인부/관리자 + 공사장 소품, 온라인 CC0) | 🔒 방향 확정(§3) | 2 | S2 |
| **S6** | 부가 요소 (자재/보급·사고 이벤트·업그레이드·사기 등 선별) | ✏️ 후보(§5.2) | 3 | S1 |
| **S7** | 오디오 & 보이스 (SFX + 캐릭터 TTS) | 🔒 방향 확정(§4) | 3 | S2 |
...
### 5.2 S6 — 부가 요소 (후보, 추후 선별)
자재/보급(자재 떨어지면 작업 중단) · 사고 이벤트(낙하·정전 등 랜덤) · 업그레이드(장비·현장 개선) · 사기/모랄 게이지 · 교대/낮밤 · 날씨. **YAGNI** — S1–S4 안정화 후 1–2개만 선별.
